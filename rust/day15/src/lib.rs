...
pub fn part1(input: String, sample: bool) -> i32 {
...
pub fn part2(input: String, sample: bool) -> u64 {
...
pub fn part2_parallel(input: String, sample: bool) -> u64 {
    let (map, _beacons) = parse(input);
    let max = if sample { 20 } else { 4000000 };
    (0..=max)
        .into_par_iter()
...
mod tests {
    ...
    #[test]
    fn test_part2_sample_parallel() {
        let contents = fs::read_to_string("test1").expect("File not found");
        assert_eq!(part2_parallel(contents, true), 56000011);
    }
    ...
    fn test_part2_parallel() {
        let contents = fs::read_to_string("input1").expect("File not found");
        assert_eq!(part2_parallel(contents, false), 13639962836448);
    }
}
